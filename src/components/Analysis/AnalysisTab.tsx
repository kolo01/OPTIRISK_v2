...
import { ChevronRight, Users, Save, AlertCircle, Eye, EyeOff, Bell, Plus } from 'lucide-react';
...
const AnalysisTab: React.FC = () => {
  ...
  const updateAnalysisData = (updates: Partial<Analysis>) => {
    ...
    AuditLogger.log(user?.id || 'unknown', 'ANALYSIS_UPDATED', `Atelier ${currentWorkshop} mis à jour`);
  };
  ...
  const canProceedToNext = () => {
    switch (currentWorkshop) {
      case 1:
           let completed = 0;
        if (analysisData.workshop1.config.contexte && analysisData.workshop1.config.contexte.length > 0) {
          completed++;
        }
        if (analysisData.workshop1.config.standard && analysisData.workshop1.config.standard.length > 0) {
          completed++;
        }
      case 2:
  ...
  const validateCurrentWorkshop = () => {
    ...
    const message = validationMessages[currentWorkshop as keyof typeof validationMessages] || 'Atelier validé';
  ...
  const saveAnalysis = () => {
    ...
    if (analysisData.id === '1') {
      ...
      AnalysisService.updateAnalysis(analysisData.id, analysisData).then((result) => {
        if (result) {
          setAnalysisData(result);
  ...
  const renderCurrentWorkshop = () => {
    ...
    switch (currentWorkshop) {
      ...
      case 5:
        return <Atelier5 {...props} onNavigateToReports={onNavigateToReports} />;
      default:
        return <Atelier1 {...props} />;
    }
  };
  ...
  if (analysisData.workshop1.config.contexte && analysisData.workshop1.config.contexte.length > 0) completed++;
  if (analysisData.workshop1.config.standard && analysisData.workshop1.config.standard.length > 0) completed++;
    ...
    <div className="max-w-7xl mx-auto space-y-6 p-4">
      ...
      <div className="ebios-card">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-100">Analyse EBIOS Risk Manager</h1>
            <p className="text-slate-400 mt-1">Méthode d'appréciation et de traitement des risques numériques</p>
          </div>
          <div className="flex items-center space-x-3">
            <button
              onClick={() => addNotification('Mode validation activé', 'info')}
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-slate-400">{Math.round(getProgressPercentage())}%</span>
          </div>
        ...
        <div className="mb-6">
          ...
          >
            ...
            <option value="Public & Administration">🏛️ Public & Administration</option>
          ...
          <div className="space-y-2">
            ...
            {analysisData.analysts.length === 0 && (
              <div className="text-center py-4 border border-dashed border-slate-600 rounded-lg">
                <p className="text-slate-500 text-sm">Aucun analyste assigné</p>
                <button
                  onClick={addAnalyst}
        ...
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-slate-100 mb-4">Ateliers EBIOS RM</h3>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="text-xs text-slate-500">
                      {index + 1 === currentWorkshop ? 'En cours' : 'À venir'}
                    </div>
                    {currentWorkshop > index + 1 && (
      ...
      <div className="ebios-card">
        ...
        <div className="p-6 border-t border-slate-700 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
          ...
          <div className="flex items-center space-x-4">
            {!canProceedToNext() && currentWorkshop < 5 && (
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={handleNext}
                disabled={currentWorkshop === 5 || !canProceedToNext()}
