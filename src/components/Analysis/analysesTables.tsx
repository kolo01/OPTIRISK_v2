...
export default function AnalysesTableau() {
  ...
  const ShowAnalyse = (slug: string) => {
    ...
    // analysisService.getOneAnalyse(slug).then((res) => {
    //   console.log(res.data);
    //   setSelectedAnalyse(res.data);
    // });
    // console.log(`Afficher les détails de l'analyse avec le slug: ${slug}`);
    window.location.href = `/showanalysis/${slug}`;
  };

  const EditAnalyse = (slug: string) => {
    // Logique pour éditer l'analyse
    // console.log(`Éditer l'analyse avec le slug: ${slug}`);
    window.location.href = `/edit-analysis-tab/${slug}`;
  };

  const DeleteAnalyse = (slug: string) => {
    // Logique pour supprimer l'analyse
    confirm("Êtes-vous sûr de vouloir supprimer cette analyse ?") &&
      analysisService.deleteOneAnalyse(slug).then(() => fetchAnalyses());
  };
  ...
  return (
    <div className="ebios-card">
      <div className="flex justify-between mb-4">
        ...
        <button
          onClick={() => {
            window.location.href = "/analysis-tab";
          }}
        ...
        <tbody>
          {analyses.map((analyse: any) => (
            <tr key={analyse.slug} className="border-t">
              <td className="px-4 py-2">{analyse.title}</td>
              ...
              <td className="px-4 py-2">
                <button
                  className="bg-blue-500 text-white px-3 py-1 rounded-md hover:bg-blue-600"
                  onClick={() => ShowAnalyse(analyse.slug)}
                >
                  Voir
                </button>
                {analyse.progress_percentage < 100 && (
                  <button
                    className="bg-green-500 text-white px-3 py-1 rounded-md hover:bg-green-600 ml-2"
                    onClick={() => EditAnalyse(analyse.slug)}
                  >
                    Terminer
                  </button>
                )}
                ...
                <button
                  ...
                  onClick={() => DeleteAnalyse(analyse.slug)}
                >
                  Supprimer
                </button>
              </td>
